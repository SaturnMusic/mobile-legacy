...
public class Deezer {
    ...
    public Pair<String, Boolean> getTrackUrl(String trackId, String md5origin, String mediaVersion, int quality) {
        ...
        if (quality != 1) {
            try {
                JSONObject songData = callGWAPI("song.getData", "{\"sng_id\": \"" + trackId + "\"}");
                JSONObject userData = callGWAPI("deezer.getUserData", "{}");
                
                System.err.println("SNGDATA" + songData);
                System.err.println("USRDATA" + userData);

// Extracting song data
String trackToken = songData.getJSONObject("results").getString("TRACK_TOKEN");

System.err.println("TRCKTKN" + trackToken);

// Extracting user data
JSONObject lcnsetknres = userData.getJSONObject("results");
System.err.println("lcnsetknres: " + lcnsetknres);
JSONObject lcnsetknusr = lcnsetknres.getJSONObject("USER");
String licenseToken = lcnsetknusr.getJSONObject("OPTIONS").getString("license_token");

System.err.println("LCNSETKN" + licenseToken);
                ...
                String data = POST("https://media.deezer.com/v1/get_url", jsonString, this.arl);

                System.err.println(data);
    
                JSONObject responseJson = new JSONObject(data);
                JSONObject json = responseJson.getJSONArray("data").getJSONObject(0).getJSONArray("media").getJSONObject(0).getJSONArray("sources").getJSONObject(0);
                return new Pair<>(json.getString("url"), true);
            } catch (Exception e) {
                e.printStackTrace();
        ...
        // Normal url gen
        return new Pair<>(Deezer.generateTrackUrl(trackId, md5origin, mediaVersion, quality), true);
    }
