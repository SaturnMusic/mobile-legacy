...
class _LyricsScreenState extends State<LyricsScreen> {
  ...
  void initState() {
    ...
    // Enable visualizer if lyricsVisualizer setting is true
    if (settings.lyricsVisualizer) {
      playerHelper.startVisualizer();
    }

    // Set up a periodic timer to update the current lyric index
    _timer = Timer.periodic(Duration(milliseconds: 350), (timer) {
      ...
      if (_currentIndex <= 0 || _prevIndex == _currentIndex) return;
      _prevIndex = _currentIndex;
      _controller.animateTo(
        // Lyric height, screen height, app bar height
    ...
    // Track change listener to exit lyrics
  ...
  void dispose() {
    ...
    // Cancel the media item subscription if it's active
    _mediaItemSub?.cancel();

    // Stop the visualizer if lyricsVisualizer setting is true
    if (settings.lyricsVisualizer) {
